 ...
 */
export function Histogram({ bins, width = 640, height = 160, xUnit = "%", marks = [] }: {
  bins: [number, number][]; width?: number; height?: number; xUnit?: string; marks?: { x: number; label: string }[];
}) {
  ...
  return (
    <svg className="chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="histogram">
