 ...
 * NaN values (a perturbation that failed to run) print as a dash on a plain cell. `caption` prints a
 * plain sentence under the grid saying what the rows, columns and cells are.
 */
export function Heatmap({ rows, cols, values, cellW = 78, cellH = 26, labelW = 200, digits = 2, caption }: {
  rows: string[]; cols: string[]; values: (number | null)[][]; cellW?: number; cellH?: number; labelW?: number; digits?: number; caption?: string;
}) {
  ...
  return (
    <figure style={{ margin: 0 }}>
      <svg className="chart" viewBox={`0 0 ${width} ${height}`} style={{ maxWidth: width }} role="img" aria-label={caption ?? "parameter sensitivity"}>
        {cols.map((c, j) => (
