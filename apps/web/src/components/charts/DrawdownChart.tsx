 ...
 */
export function DrawdownChart({ equity, width = 640, height = 140, caption }: { equity: number[] | [number, number][]; width?: number; height?: number; caption?: string }) {
  ...
  return (
    <figure style={{ margin: 0 }}>
      <svg className="chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={caption ?? "drawdown from peak, percent"}>
        {ticks(yd, 2).map((v) => (
          <g key={v}>
        ...
        <text x={width - PAD.right} y={height - 4} textAnchor="end">trade {dd.length}</text>
      </svg>
